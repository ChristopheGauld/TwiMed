...
Q <- qgraph(cor_matrix_reduite, layout = "spring", posCol = "blue", negCol = "NA",
            ...
            label.scale = FALSE,
            minimum = 0.005, # Ne pas afficher les edges avec faible corrélation pour faciliter le chargement graphique
            #repulsion = 100, # Augmenter la distance entre les noeuds pour améliorer la visualisation en "cluster"
            legend.mode = "groupe",sampleSize = nrow(cor_matrix_reduite), alpha = 0.05, # Pour ne prendre en compte que les corrélations statistiquement significatifs, en tenant compte de l'inflation du risque alpha par les tests multiples via la méthode de Bonferroni.
            palette = "pastel", vsize = 2)
